...
This is a containerized environment that implements a standard development environment built with [docker-compose](https://docs.docker.com/compose/) for the [Fieldsets IAC Framework](https://github.com/fieldsets/fieldsets).

This repository is multi purpose. Utilizing both this repository [Dockerfile](./Dockerfile) and [docker ompose file](./docker-compose.yml), this codebase can serve as a boiler plate for containerized deployments of any down stream integrations of this framework. This repository can also serve as a localized workspace that helps us maintain consistency between versions and dependencies within multiple services.
...
Currently this environment is a work in progress focusing on aggregated metrics and partitioning of data, but has been created with the idea that all of the branches of development can utilize this environment such that developers within each branch can easily onboard and contribute to other projects when necessary.
